...
public struct ThemeLoaderService {
    ...
    public func discoverThemes() -> ThemeLoadResult {
        ...
        for url in bundledThemeFileURLs() {
            ...
            case .failure(let message):
                warnings.append(ThemeLoadWarning(sourceURL: url, message: message))
            }
        }
    ...
    private func bundledThemesDirectoryCandidates() -> [URL] {
        ...
        if let bundleURL = bundle.bundleURL {
            candidates.append(contentsOf: siblingResourceBundleThemeDirectories(in: bundleURL))
        }
    ...
    private func parseThemeFile(at url: URL) -> Result<ThemeDefinition, String> {
        do {
            let data = try Data(contentsOf: url)
            let definition = try JSONDecoder().decode(ThemeDefinition.self, from: data)
            return .success(definition)
        } catch {
            return .failure("Could not parse theme: \(error.localizedDescription)")
        }
    }
