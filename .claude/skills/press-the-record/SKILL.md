...
| | `main` (stable) | `v2` (beta) |
|---|---|---|
| Audience | everyone | the maintainer only |
| Marketing version | patch bump | stays `2.0.0` |
| `AppVersion.channel` | `""` | `"BETA"` |
| Tag | `v1.5.11` | `v2.0.0-beta.<build>` |
| GitHub release | normal | `--prerelease` |
| Feed | `website/appcast.xml` | `website/appcast-beta.xml` |
| CHANGELOG section | new per release | one `## 2.0.0` that accumulates |
...
- **Marketing version (v2):** stays `2.0.0`. Bump `channelOrdinal` instead — it
  is the beta number in the About pill, so beta 3 is `channelOrdinal = "3"`.
- `channel` stays `""` on main and `"BETA"` on v2 unless the user says otherwise.
...
- `Packaging/CrateDiggerApp/Info.plist`: `CFBundleShortVersionString` (= marketing)
  and `CFBundleVersion` (= build). On v2 only `CFBundleVersion` moves, and do
...
**On v2, step 3 is different.** There is one `## 2.0.0 (<build>) — <date>`
section at the top of the CHANGELOG that you *add to* each beta rather than
replacing, since a tester wants to read everything in 2.0.0 so far. Keep the
heading matching `## 2.0.0 ` exactly — the appcast script finds the notes by
that prefix. Do **not** touch `README.md` or `website/index.html`: those
describe the shipping stable release, which is still 1.5.x.
...
- `git fetch origin` then `git push origin <branch>` — `main` for a stable
  release, `v2` for a beta. The website redeploys via GitHub Pages on a push to
  `main` that touches `website/`; a `v2` push deploys nothing.
...
- **Tag:** `v<marketing>` on main, `v2.0.0-beta.<build>` on v2, created by this
  command against the branch you released from (pass `--target main` or
  `--target v2`; a raw commit SHA is rejected as an invalid `target_commitish`).
- **On v2, pass `--prerelease`.** Without it the beta becomes
...
- **Body:** the CHANGELOG entry for this version — intro line, then
  ...
  contain backticks and em-dashes that a shell-quoted `--notes` mangles.
...
# v2 beta instead:
gh release create v2.0.0-beta.<build> dist/CrateDigger-2.0.0.dmg \
  --target v2 --prerelease \
  --title "CrateDigger 2.0.0 beta <ordinal> (build <build>) — <tagline>" \
  --notes-file <notes.md>
```
...
# v2 beta: its own feed, and a tag that cannot be derived from the DMG name.
scripts/update-appcast.sh \
  --appcast website/appcast-beta.xml \
  --tag v2.0.0-beta.<build>
```

**Publishing the beta feed takes one extra move.** GitHub Pages only deploys
`website/` from `main`, so a feed sitting on `v2` is never served. Take just
that one file across, and nothing else:

```bash
git checkout main
git checkout v2 -- website/appcast-beta.xml
git commit -m "chore(release): publish 2.0.0-beta.<build> to the beta feed"
git push origin main
git checkout v2
```

`main` never receives beta code this way — only the beta feed, which no stable
build reads. Confirm `website/appcast.xml` is untouched by the commit
(`git show --stat`); if it appears, stop, because that is the file 1.5.x users
read.

It signs the DMG with the private EdDSA key in the Keychain and rewrites
`website/appcast.xml`, taking release notes from the CHANGELOG section for this
version. Then commit and push it:

`chore(release): publish <marketing> to the update feed`

Confirm the entry landed: `grep -c "<item>" website/appcast.xml` went up, and
the new `<enclosure url=...>` names this version's DMG. If the key is missing
the script fails loudly — stop and report rather than pushing a feed with an
unsigned entry, which every installed copy will refuse.
...
Upload whatever landed in `dist/updates/`, then check the whole feed rather than
just the entry you added — an older item can be repointed at a tag whose asset
was never uploaded:

```bash
# no deltas is normal on a first release; only upload when some exist
ls dist/updates/*.delta >/dev/null 2>&1 && gh release upload v<marketing> dist/updates/*.delta
